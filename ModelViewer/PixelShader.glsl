...
in vec3 vecToCameraInTangentSpace;
in vec3 lightDirectionInTangentSpace[MAX_LIGHTS];
in vec3 vecToLightInTangentSpace[MAX_LIGHTS];
...
void main () 
{	
	vec4 offset = texture( heightTex, textureCoord );
	offset = offset * scale - bias;
	
	if( ( debug & useParallax ) != 0 )
	{
		offset = vec4( 0, 0, 0, 0 );
	}

	vec3 cameraNormalized = normalize( vecToCameraInTangentSpace );
	vec2 newTexCoord = cameraNormalized.xy * offset.xy + textureCoord;
	vec3 normal = normalize( normal_vector );

    vec4 diffuseColor = texture( diffuseTex, newTexCoord );
	...
	for( int i = 0; i < MAX_LIGHTS; ++i )
	{
		...
		vec3 vecToLightNormalized = normalize( vecToLightInTangentSpace[i] );
		...
		if( ( debug & useLighting ) == 0 )
		{
			...
			{
				diffuseIntensity = clamp( dot( normalFromTexture, vecToLightNormalized ), 0, 1 );
				float dist = length( vecToLightInTangentSpace[i].xyz );
...
/*/
				float spotlightAngle = dot( -vecToLightNormalized, normalize( lightDirectionInTangentSpace[i] ) );
...
//*/
		...
		{
			float specularIntensity = dot( normalize( cameraNormalized ), reflect( normalFromTexture, vecToLightNormalized ) );
    ...
    fragColor = totalLightColor * diffuseColor * max( totalDiffuseIntensity, 0 ) + emissiveIntensity * texture( emissiveTex, newTexCoord ) + ( max( totalSpecularIntensity, 0 ) * texture( specularTex, newTexCoord ) );
